...
class MrpBom(models.Model):
    ...
    def custom_print_bom_structure_excel(self):
        ...
        title_style1_table_head_right = xlwt.easyxf('align: horiz right; font: name Times New Roman,bold on, italic off, height 200')
        title_style1_table_value_left = xlwt.easyxf('font: name Times New Roman,bold off, italic off, height 200')
        title_style1_table_value_right = xlwt.easyxf('align: horiz right; font: name Times New Roman,bold off, italic off, height 200')
        ...
        for product_variant_id in candidates:
            data = self.env['report.mrp.report_bom_structure']._get_pdf_line(bom_id.id, product_id=product_variant_id, qty=quantity, unfolded=True)
        ...
        attach_id = self.env['custom.mrp.bom.structure.excel'].create({'name':'BOM Structure.xls', 'xls_output': base64.encodestring(stream.getvalue())})
        return {
